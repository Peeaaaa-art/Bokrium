...
- 未設定の場合は匿名リクエストにフォールバックしますが、検索は失敗し続けます。
  ...
  | **読書メモ**には、**Markdown対応のTipTapエディタ**を採用。見たまま編集できる直感的なUI。さらに、タグ付けや書籍情報の編集、「読書中・読みたい・読了」といったステータスの設定にも対応。タグ付けはTurbo Streamによる非同期更新に対応しており、モーダルを閉じずに複数のタグを続けて選択できます。
  ...
  **モーダル表示**では、メモの文字数や作成日・更新日も確認できます。保存ボタンの自動制御や改行仕様にも細かく対応し、ストレスのない編集体験を提供しています。 |
  ...
  <br><h3 align="center">📓 手書きノート機能</h3>

  | 📓 Excalidrawによる自由描画ノート（1冊に複数枚・サムネイル一覧） |
  |-------------------------|
  | <!-- スクリーンショット/GIFをここに挿入 --> |
  | 本の詳細ページから、ペンアイコンをタップすると**Excalidraw**[^4]ベースの自由描画キャンバスが開き、手書きでノートを作成できます。1冊につき何枚でもノートを作成可能で、サムネイル一覧から一覧・追加・削除ができます。ノート内容は1.2秒のデバウンス後に自動保存され、ページ離脱時（画面遷移・タブ切り替え等）にも保存を取りこぼさないよう明示的にフラッシュ処理を行っています。保存後にはサムネイル画像を自動生成し、一覧表示に利用します。<br>iPad・スタイラスでの利用を主眼としており、スマートフォンでは閲覧専用モードで表示されます。 |

  <br><h3 align="center">🗺 読書戦略ボード</h3>

  | 📋 かんばん / 🗓 ロードマップ / 📃 リストの3ビューで読書進捗を管理 |
  |-------------------------|
  | <!-- スクリーンショット/GIFをここに挿入 --> |
  | 読書中の本について、**読了目標日・現在ページ・必要ペース（残りページ÷残り日数）**を可視化し、計画的な読書をサポートする専用ボード。用途に応じて3つのビューを切り替えられます。<br>**リスト**：読了目標日が近い順に並び、進捗バーとペースをカードで表示。<br>**かんばん**：「読みたい・読書中・読了」の3列をドラッグ&ドロップでステータス変更でき、列内での手動並び替え（ドロップ位置の保持）にも対応。<br>**ロードマップ**：開始日→読了目標日を時間軸バーで表示し、バーの中央をドラッグすると期間ごと移動、端をドラッグすると開始日・目標日を個別に伸縮できます。 |
  ...
  <br><h3 align="center">📤 データエクスポート</h3>

  | 📑 蔵書・メモをCSVで一括エクスポート |
  |-------------------------|
  | <!-- スクリーンショット/GIFをここに挿入 --> |
  | マイページから、登録済みの本とメモをまとめてCSVでダウンロードできます。1冊1行の形式で、タイトル・著者・ISBN・出版社・ページ数・価格・ステータス・タグ・メモ内容（複数メモは結合）を出力し、自分の読書データをバックアップ・外部活用したい場合に利用できます。 |
  ...
  | 手書きノート   | Excalidraw[^4]（Reactベース 描画キャンバス、ページ専用entrypointで分離バンドル）                              |
  | 読書戦略ボード | SortableJS（かんばんのドラッグ&ドロップ）、Pointer Eventsによる自前実装（ロードマップのバードラッグ）        |
  | 通知機能       | LINE Messaging API、ActionMailer + cron-job.org（定期メモ通知）                                   |
  | 支援機能       | Stripe（寄付・マンスリーサポート）                                                                           |
  | データ出力     | CSVエクスポート（蔵書・メモを一括ダウンロード）                                                              |
  ...
  - CSRF対策としてPOSTリクエストにトークン検証を適用（Rails標準）
  - XSS対策として `sanitize` メソッドを使用し、危険なHTMLの出力を防止
  - `Strong Parameters` により許可されたパラメータのみを受け付ける構成
  - ActiveRecord の`パラメータ化クエリ`で SQLインジェクションを防止
  - GitHub Actions に `Brakeman` を組み込み、CI/CD内で静的解析による継続的な脆弱性チェックを実施
  - GitHub `Dependabot` による脆弱性のある依存Gemの自動検出・通知を活用し、迅速なアップデート対応を実施
  - Github `Code scanning` を有効化し、セキュリティリスクやコードの問題を自動チェック
  - Github `Secret scanning` により、誤ってコミットされたトークンや鍵を即座に警告
  - Fly.ioのエッジネットワークと自動HTTPS対応により、アプリ全体の通信はTLSで暗号化
  - Cloudflare の `マネージドチャレンジ` をログイン・新規登録画面に適用し、Botや不正リクエストを事前ブロック
  ...
  ## 📡 インフラ構成図
  [![インフラ構成図](https://i.gyazo.com/8e544dea5023280e7015b97213ee07fb.png)](https://gyazo.com/8e544dea5023280e7015b97213ee07fb)
  ...
  >#### データベースは Supabase、ストレージは Cloudflare R2 を採用
  
  データベースには、PostgreSQL をベースとしたマネージドサービスである **[Supabase](https://supabase.com)** を採用しています。  
  Fly.io の日本リージョンと組み合わせることで、低レイテンシかつ安定したデータアクセスを実現できる点が大きな決め手でした。
  
  ストレージには当初 S3 を使用していましたが、転送量課金のない **Cloudflare R2** へ移行しました。  
  S3 互換 API によりスムーズな切り替えが可能で、Cloudflare CDN との統合による高速な配信も実現しています。
  ...
  支援機能としては、Stripe による単発寄付（`donations`）と継続支援（`monthly_supports`）を実装。<br>
  手書きノートは `handwritten_notes`（本・ユーザーに紐づき、シーンデータとサムネイルをActive Storageで管理）で1冊複数枚を実現。<br>
  読書戦略ボードは `books` テーブルの `started_on` / `target_finish_on` / `current_page` / `board_position` で開始日・目標日・進捗・かんばん内の並び順を管理。
  ...
  Bokrium では、読書メモや書籍情報を対象とした日本語の全文検索を重視しており、pg_search[^3] を活用できる PostgreSQL の全文検索機能は必須でした。  
  また、Rails との高い親和性に加え、安定した同時接続処理が求められる点からも、SQLite や MySQL ではなく PostgreSQL が最適と判断しました。
  
  本番環境では、これらの要件を満たしつつ、スケーラブルで開発体験に優れたマネージドデータベースとして **Supabase** を採用しています。  
  Fly.io の日本リージョンと組み合わせることで、低レイテンシかつ安定したデータアクセスを実現しています。
  ...
  [^4]: [Excalidraw](https://github.com/excalidraw/excalidraw) は、手描き風の見た目を持つオープンソースの描画・ホワイトボードライブラリです。Bokriumでは手書きノート機能専用のVite entrypointとして組み込み、他ページのバンドルサイズに影響させない構成にしています。
