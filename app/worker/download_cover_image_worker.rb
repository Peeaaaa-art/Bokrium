# app/workers/download_cover_image_worker.rb
require "open-uri"
...
class DownloadCoverImageWorker
  ...
  def perform(book_id, image_url)
    ...
    begin
      ...
      Rails.logger.info "[DownloadCoverImageWorker] 画像を保存しました：#{filename}"
    rescue => e
      Rails.logger.error "[DownloadCoverImageWorker] BookID=#{book_id} 画像保存失敗: #{e.message}"
    end
  end
end
